...
//    数据变换   5个数据
    ...
    protected void map(Object key, Text value, Context context) throws IOException, InterruptedException {
        ...
        if(stringTokenizer.hasMoreTokens()) {
            ...
            // 第一列和第五列共同作为 key，中间用制表符隔开，value为空即可
             
            String key_first=stringTokenizer.nextToken();
            String key_second=stringTokenizer.nextToken();
            String key_third=stringTokenizer.nextToken();
            String key_forth=stringTokenizer.nextToken();
            String key_fifth=stringTokenizer.nextToken();
             
            outKey.set(key_first + "\t" + key_second + "\t" + key_third + "\t" + key_forth+ "\t" + key_fifth);
...
class DualSortPartition extends HashPartitioner<Text, Text> {
    ...
    public int getPartition(Text key, Text value, int numReduceTasks) {
        ...
        // key是 1~max 的数，key的空间大小为max，假设key是均匀分布的
        if(key_first < 1) return 0; // not reachable
        if(key_first > max) return numReduceTasks - 1; // not reachable
        int size = max / numReduceTasks; // 每一台机器处理的key值的数量  
...
class DualSortCompare extends WritableComparator {
    ...
    public int compare(WritableComparable a, WritableComparable b) {
        ...
        if(a_first != b_first) {
            ...
            String a_fifth=a_tokens[4].split(" ")[1].replaceAll(":","");
            String b_fifth=b_tokens[4].split(" ")[1].replaceAll(":","");
             
            int a_fifth_int = Integer.parseInt(a_fifth);
            int b_fifth_int = Integer.parseInt(b_fifth);
            return Integer.compare(a_fifth_int, b_fifth_int);
        }
    }
}
